...
USE K003_2
IF OBJECT_ID('tempdb..DistribucionDePagos') IS NOT NULL  
   BEGIN  
    DROP TABLE DistribucionDePagos;
END
CREATE TABLE DistribucionDePagos (
...
WHERE PRE.IDKATIOS='DELTA' 
AND PRE.PRODUCTO NOT IN ('Venta Cuota Creciente','Venta de Flujos')
AND (CASE WHEN 'FECHA_EFECTIVA'='' THEN CAST(DRO.FECHAEFECTIVA AS DATE) WHEN 'FECHA_APLICACION'='FECHA_APLICACION' THEN CAST(DRO.FECHADIA AS DATE) END) BETWEEN '2022-01-01' AND CONVERT(VARCHAR(10), GETDATE(), 120)
...
INSERT INTO DistribucionDePagos
SELECT
